...
Delegation tools are the third exception: `push-task`, `resume-task`, and `task-ask` (from pi-tree-like-subagent) let a planner hand a self-contained chunk of work to an isolated task branch, or suspend work until you answer, instead of doing it inline. They are default-on but come with one caveat worth knowing: a task branch runs in its own session with its own tool policy, so pushing an implementation task from Plan mode still edits files — in that branch, not here. Plan mode's write gate constrains this session's tool calls, not the work it delegates.
